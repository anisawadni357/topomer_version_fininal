        ...
        if (data.clientSecret) {
          try {
            ...
            if (error) {
             ...
             if(this.type==2)
             {
             if(sessionStorage.getItem('livraison_nom'))
             {
              this.livraisonNom=sessionStorage.getItem('livraison_nom') ?? "";
             }
             if(sessionStorage.getItem('livraison_prenom'))
              {
               this.livraisonPrenom=sessionStorage.getItem('livraison_prenom') ?? "";
              }
              if(sessionStorage.getItem('livraison_adresse'))
                {
                 this.livraisonAdresse=sessionStorage.getItem('livraison_adresse') ?? "";
                }
              }
             let data={
              "user":this.id,
              ...
              "notes":"vide",
              "type":this.type,
              "livraison_nom":this.livraisonNom,
              "livraison_prenom":this.livraisonPrenom,
              "livraison_adresse":this.livraisonAdresse
             }
